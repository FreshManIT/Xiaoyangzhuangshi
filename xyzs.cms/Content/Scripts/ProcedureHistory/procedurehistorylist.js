...
function LoadingActivityResultDetailDate() {
    ...
    $.ajax({
        url: hidRootUrl + "/ProcedureHistory/ProcedureHistoryListPage",
        type: "POST",
...
function getAllUser() {
    $.ajax({
        url: hidRootUrl + "/ProcedureHistory/GetAllUser",
        type: "POST",
        success: function (data) {
            if (data && data.ResultCode == 0) {
...
function saveDataInfo() {
    //加载
    $(".loading-container").removeClass("loading-inactive");
    $.ajax(hidRootUrl + "/ProcedureHistory/SaveProcedureResourceInfo", {
        type: "POST",
